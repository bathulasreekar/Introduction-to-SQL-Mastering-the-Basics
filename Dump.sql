...
INSERT INTO IPL_BIDDER_DETAILS VALUES (102,'USR002','Krishan Valimbe',7088118461,
'valimbek@yahoo.com',NULL),(103,'USR003','Megaduta Dheer',9325049602,'dheerm@gmail.com',NULL),
...
-- Dump completed
select * from IPL_Team_Standings where matches_won/matches_played;
select Stadium_name from IPL_Stadium group by City;
select* from IPL_Match;
select * from IPL_Match where TOSS_WINNER/MATCH_WINNER;

select s1.Bid_team,s2.No_of_bids,s2.No_of_matches
from IPL_Bidding_Details as s1
inner join IPL_Bidder_Points as s2
 where sum(No_of_bids);
 
 select team_id from IPL_Team_Standings where max(Matches_won);
 
 select sum(total_matches),sum(matches_won),sum(matches_loss) from IPL_Team_Standings group by Teamid;
 
 select s1.teamid, s1.team_name, s2.player_name,S2.Player_role
 from IPL_Team as s1 
 inner join IPL_Team_Players as s2 
 where s1.team_name ='m%' and s2.player_role = 'bowler';
 
 select s1.teamid, s1.team_name, s2.player_name,S2.Player_role
 from IPL_Team as s1 
 inner join IPL_Team_Players as s2 
 where desc s2.player_role = 'allrounder' >4;
